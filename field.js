const hat = '^';              // 🎯 the goal
const hole = 'O';             // 💀 deadly pit
const fieldCharacter = '░';   // walkable empty ground
const pathCharacter = '*';    // the player + the trail left behind
...
class Field {
  ...
  static generateField(height = 10, width = 10, holePercentage = 0.2) {
    // 1. Fill the whole grid with walkable ground.
    const grid = Array.from(
      { length: height },
      () => Array(width).fill(fieldCharacter)
    );

    // 2. Build a list of every cell, then Fisher–Yates shuffle it.
    const cells = [];
    ...
    // 3. Hand out cells in order: holes first, then hat, then actor.
    let idx = 0;
  ...
  _move(dx, dy) {
    ...
    // --- Out of bounds: lose immediately, do NOT move ---
    ...
    // --- Landed on a hole: lose (keep 'O' visible so the map shows it) ---
    if (target === hole) {
      return { status: 'hole' };
    }

    // --- Landed on the hat: win (keep '^' visible) ---
    if (target === hat) {
      return { status: 'hat' };
    }

    // --- Safe ground: mark the new position as the player ---
